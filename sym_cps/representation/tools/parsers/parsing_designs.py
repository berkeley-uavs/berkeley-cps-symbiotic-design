...
from sym_cps.representation.shared.paths import design_library_root_path_default
...
from sym_cps.representation.tools.parsers.temp_objects import (
    all_connectors,
    all_parameters,
    connectable_components_classes,
        ...
        if os.path.isdir(design_path):
            ...
            for entry in instance_connectors:
                ...
                if (
                    ...
                    in connectable_components_classes.keys()
                ):
                    connectable_components_classes[from_component_class.comp_type].add(
                        to_component_class.comp_type
                    )
                else:
                    connectable_components_classes[from_component_class.comp_type] = {
                        to_component_class.comp_type
                    }
            ...
            for instance, (
                ...
                for name, (value, design_parameter) in parameters_floats.items():
                    ...
                    parameter = Parameter(value, new_component, param_type)
...
def parse_design_from_design_swri(path: Path, library: Library) -> DConcrete:
    ...
    for component_data in design_swri_data["components"]:
        component_instance = component_data["component_instance"]
        component_choice = component_data["component_choice"]
        component_map[component_instance] = (component_choice, {})
    ...
    for parameter_data in design_swri_data["parameters"]:
        ...
        for prop in component_properties:
            ...
            component_choice = component_map[component_name][0]
            param_id = parameter_id(
                component_property, str(library.components[component_choice].comp_type)
            )

            param_type = all_parameters[param_id]
            parameter = Parameter(parameter_value, component_choice, param_type)
            parameters[param_id] = parameter
    ...
    for component_name, (component_choice, parameters) in component_map.items():
        new_component = Component(
            id=component_name,
            library_component=library.components[component_choice],
            parameters=parameters,
        )
        new_design.add_node(new_component)
    """Read the connection"""
    for connection_data in design_swri_data["connections"]:
        ...
        connection = Connection(
            component_a=new_design.get_instance(instance_s),
            connector_a=all_connectors[connector_id_s],
            component_b=new_design.get_instance(instance_t),
            connector_b=all_connectors[connector_id_t],
        )
        try:
            new_design.connect(connection)
        except:
