...
def parse_trim_state(f, turning=False):
    ...
    if turning:
        ...
        steady_state.update(
            parse(
                "Objective steady state speed is UVW world = {speed:>g} {:>g} {:>g} m/s; PQR world =  {:>g} {:>g} {rotation:>g} rad/s",
                next(f).strip(),
            ).named
        )
        steady_state.update(
            parse(
                "Turning radius {turning_radius:>g} m (positive means clockwise looking down), Rworld = {turning_rate:>g} radians/s",
                next(f).strip(),
            ).named
        )
    else:
        steady_state.update(
            parse(
                "Objective steady state speed is UVW world = {speed:>g} {:>g} {:>g} m/s = {:>g} {:>g} {:>g} miles per hour",
                next(f).strip(),
            ).named
        )
    ...
    while "Total" not in line:
        if "Warning" in line or "Caution" in line:
            ...
            if (
                battery := parse(
                    "Battery # {number:>d} Current = {current:>g} amps,  Time to 20% charge remaining = {flight_seconds:>g} s,  Flight distance = {distance:>g} m",
                    line.strip(),
                )
            ) is None:
                battery = parse(
                    "Battery # {number:>d} Current = {current:>g} amps,  Time to 20% charge remaining = {flight_seconds:>g} s,  Flight distance = {distance:>g} m with warning",
                    line.strip(),
                )
    ...
    total_power = parse(
        "Total power from batteries {batteries:>g} watts;  total power in motors {motors:>g} watts", line.strip()
    ).named
...
def fdm_to_json(f):
    ...
    motor_info = ascii.read(
        motor_table, format="fixed_width_no_header", col_starts=motor_columns, names=motor_column_names
    )
    ...
    lift_info = ascii.read(
        lift_table, format="fixed_width_no_header", col_starts=lift_columns, names=lift_and_drag_column_names
    )
    ...
    drag_info = ascii.read(
        drag_table, format="fixed_width_no_header", col_starts=drag_columns, names=lift_and_drag_column_names
    )
    ...
    while (line := next(f)) != "\n":
        if (
            propeller := parse(
                "Propeller motor {motor:>d} controlled by uc channel {channel:>d} and powered by battery {battery:>d}",
                line.strip(),
            )
        ) is not None:
            ...
            wing := parse(
                "Wing {wing:>d} servo {servo:>d} controlled by uc channel {channel:>d} with bias {bias:>g}",
                line.strip(),
            )
        ) is not None:
    ...
    trims_summary_info = ascii.read(
        trims_summary_table,
        format="fixed_width_no_header",
        col_starts=trims_summary_columns,
        names=trims_summary_column_names,
    )

    max_level_distance = parse(
        "Of all cases, maximum no-warning distance was at speed {speed:>g} m/s with distance {distance:>g} m",
        next(f).strip(),
    ).named
    max_level_speed = parse(
        "Of all cases, maximum no-warning speed was a speed of {speed:>g} m/s with distance {distance:>g} m",
        next(f).strip(),
    ).named
    ...
    clockwise_trims_summary_info = ascii.read(
        clockwise_trims_summary_table,
        format="fixed_width_no_header",
        col_starts=clockwise_trims_summary_columns,
        names=clockwise_trims_summary_column_names,
    )

    max_clockwise_distance = parse(
        "Of all cases, maximum no-warning distance was at speed {speed:>g} m/s with distance {distance:>g} m",
        next(f).strip(),
    ).named
    max_clockwise_speed = parse(
        "Of all cases, maximum no-warning speed was a speed of {speed:>g} m/s with distance {distance:>g} m",
        next(f).strip(),
    ).named
    ...
    counterclockwise_trims_summary_info = ascii.read(
        counterclockwise_trims_summary_table,
        format="fixed_width_no_header",
        col_starts=counterclockwise_trims_summary_columns,
        names=counterclockwise_trims_summary_column_names,
    )

    max_counterclockwise_distance = parse(
        "Of all cases, maximum no-warning distance was at speed {speed:>g} m/s with distance {distance:>g} m",
        next(f).strip(),
    ).named
    max_counterclockwise_speed = parse(
        "Of all cases, maximum no-warning speed was a speed of {speed:>g} m/s with distance {distance:>g} m",
        next(f).strip(),
    ).named
    ...
    trajectory_table_info = ascii.read(
        trajectory_table,
        format="fixed_width_no_header",
        col_starts=trajectory_table_columns,
        names=trajectory_table_column_names,
    )
    ...
    while (line := next(f)) != "\n":
        if (
            battery := parse(
                "Battery {battery:^d} fraction capacity used {capacity_used:>g} and fraction max continuous amperage used {amperage_used:>g}",
                line.strip(),
            )
        ) is not None:
            ...
            motor := parse(
                "Motor {motor:>d} fraction max amps reached {max_amps:>g} and fraction max power reached {max_power:>g}",
                line.strip(),
            )
        ) is not None:
    ...
    complete["counterclockwise_trims_summary"] = counterclockwise_trims_summary_info.to_pandas().to_dict(
        orient="records"
    )
