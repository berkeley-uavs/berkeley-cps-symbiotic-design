...
def test_contract():
    ...
    print(f"# Motor: {num_motors}, # Battery: {num_batteries}, # Propeller: {num_propellers}, # BatteryController: {num_batt_controllers}")

    ret_val = check_contract(battery=battery, battery_controller=battery_controller, motor=motor, 
                            propeller=propeller, num_batteries=num_batteries, num_motors=num_motors, table_dict=table_dict)    
    ...
    for n_p, propeller in enumerate(c_library.components_in_type["Propeller"]):
        ...
        for n_m, motor in enumerate(c_library.components_in_type["Motor"]):
            ...
            for n_b, battery in enumerate(c_library.components_in_type["Battery_UAV"]):
                ...
                if propeller.properties["SHAFT_DIAMETER"].value > motor.properties["SHAFT_DIAMETER"].value or propeller.properties["SHAFT_DIAMETER"].value >= 30:
                    continue
                # if propeller.properties["SHAFT_DIAMETER"].value != motor.properties["SHAFT_DIAMETER"].value:
                #     continue
                    
                print("counter:", n_b * 20*100 + n_m * 100 + n_p)
                ret_val = check_contract(battery=battery, battery_controller=battery_controller, motor=motor, propeller=propeller, 
                                        num_batteries=num_batteries, num_motors=num_motors, table_dict=table_dict)
                if obj_val < ret_val:
    ...
    print(f"BatteryController: {battery_controller.id}")
    ret_val = check_contract(battery=best_battery, battery_controller=battery_controller, motor=best_motor, 
                            propeller=best_propeller, num_batteries=num_batteries, num_motors=num_motors, table_dict=table_dict)
    print(ret_val)
...
def check_contract(battery, battery_controller, motor, propeller, num_batteries, num_motors, table_dict):
    ...
    def propeller_motor_g_inst(i_motor, rho, speed, motor, propeller):
        return propeller_motor_g(i_motor=i_motor, rho=rho, speed=speed, motor=motor, propeller=propeller, table_dict=table_dict)
    ...
    for i_motor in is_motor:
        (thrust, v_motor, weight, I_motor_max, P_motor_max, shaft_motor, shaft_prop) = propeller_motor_g_inst(i_motor=i_motor, rho=rho, speed=speed, motor=motor, propeller=propeller)
    ...
    if system_requirement_g(thrusts=thrusts, vs_motor=vs_motor, v_battery=voltage, i_battery=i_battery, weights=weights,
                            I_max=I_max, is_motor=is_motor, Is_motor_max=Is_motor_max, Ps_motor_max=Ps_motor_max, 
                            shaft_motor=shaft_motor, shaft_prop = shaft_prop):
        print("Contract Satisfied")
        obj_val = objective(thrusts=thrusts, vs_motor=vs_motor, v_battery=voltage, i_battery=i_battery, weights=weights,
                                I_max=I_max, is_motor=is_motor, Is_motor_max=Is_motor_max, Ps_motor_max=Ps_motor_max)
    else:
