...
class ContractTemplate(object):
    ...
    def instantiate(self, instance_name = "", **kwargs) -> ContractInstance:
        ...
        vs = {v: z3.Real(f"{self._name}_{self._count}_{instance_name}_{v}") for v in self._vs}
...
class ContractManager(object):
    def __init__(self, property_interface_fn: Callable):
        self._c_instance = dict[str, ContractInstance] # instance name -> 
        self._clauses = []
    ...
    def add_instance(self, inst1: ContractInstance):
        self._c_instance[inst1.name] = inst1
        self._selection_candidate[inst1] = {}
        self._clauses.append(inst1._guarantee)
        self._clauses.append(inst1._assumption)
    ...
    def set_selection(self, inst: ContractInstance, candidate_list: list):
        ...
        for candidate in candidate_list:
            use_v = z3.Bool(f"{inst.name}_use_{candidate.name}")
    ...
    def solve_optimize(self,  max_iter = 0, timeout_millisecond = 100000):
        ...
        self._solver.add(self._all_clause)
        self._solver.push(self._objective_expr >= self._objective_val)
        ...
        while ret == z3.sat:
            ...
            self._solver.pop()
            self._solver.push(self._objective_expr >= new_value)
            ret = self._solver.check()
    ...
    def get_metric(self, inst_name, port_property_name):
        var = self.get_var(self, inst_name=inst_name, port_property_name=port_property_name)
        return self._model(var).numerator_as_long() / self._model(var).denominator_as_long()
    ...
    def print_metric(self):
        ...
        for inst in self._c_instance.values():
            ...
            for port_name in inst.port_name_list:
                val = self.get_metric(inst_name = inst.instance_name, port_name=port_name)
                print(f"        {port_name}: {val}")
            for property_name in inst.property_name_list:
                model_ref = self._model[inst.get_property_var(property_name)]
                val = model_ref.numerator_as_long()/ model_ref.denominator_as_long()
                print(f"        {property_name}: {val}")
        print("==================================================================")

    def print_selection_result(self, ret):
        print(f"===================Component Selection Result=====================")
        for inst, cand in ret:
            property_dict = self._property_interface_fn(cand)
            cand_name = property_dict["name"]
            print(f"    {inst.name}: {cand_name}")
        print("==================================================================")
