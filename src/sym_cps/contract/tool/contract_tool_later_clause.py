...
class ContractInstance(object):
    ...
    def create_clause(self, prop_dict: dict):
        A = self._assumption(vs=self._z3_var_dict, props=prop_dict)
        G = self._guarantee(vs=self._z3_var_dict, props=prop_dict)
        return A, G


class ContractTemplate(object):
    def __init__(
        self, name: str, port_name_list: list, property_name_list: list, guarantee: Callable, assumption: Callable
    ):
        self._port_name_list = port_name_list
        self._property_name_list = property_name_list
        self._guarantee = guarantee
...
class ContractManager(object):
    ...
    def add_instance(self, inst1: ContractInstance, no_selection=False):
    ...
    def set_selection(self, inst: ContractInstance, candidate_list: list):
        ...
        if candidate_list is None:
            print(inst.property_name_list)
            if inst.property_name_list:
                print(f"Error, Instance {inst.instance_name} requires selection.")
            else:
                A, G = inst.create_clause(prop_dict=None)
                self._clauses.extend(A)
                self._clauses.extend(G)
                return
        ...
        for candidate in candidate_list:
            ...
            A, G = inst.create_clause(prop_dict=property_dict)
            combine_clauses = A + G
            self._clauses.append(z3.Implies(use_v, z3.And(*combine_clauses)))
    ...
    def solve_optimize(self, max_iter=0, timeout_millisecond=100000, delta=0.01):
        num_iter = 0
        ...
        while ret == z3.sat:
            ...
            new_value = self.calculate_objective() + delta
            # self._solver.pop()
