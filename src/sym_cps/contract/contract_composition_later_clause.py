...
class Hackathon2Contract(object):
    ...
    def set_contract(self):
        ...
        def propeller_guarantee(vs, props):
            return [vs["torque_prop"] == props["C_p"] * vs["rho"] * vs["omega_prop"]**2 * props["diameter"] ** 5 / (2 * 3.14159265) ** 3,
                    vs["thrust"] == (props["C_t"] * vs["rho"] * vs["omega_prop"]**2 * props["diameter"] ** 4 / (2 * 3.14159265) ** 2),
                    vs["omega_prop"] >= 0]
        ...
        def motor_guarantee(vs, props):
            return [vs["I_motor"] * props["R_w"] == vs["V_motor"] - vs["omega_motor"] / props["K_v"], 
                    vs["torque_motor"] == props["K_t"]/props["R_w"] * (vs["V_motor"] - props["R_w"] * props["I_idle"] - vs["omega_motor"]/props["K_v"])]
        ...
        battery_controller_port_name_list = (["V_battery", "I_battery"] + 
                                            [ f"I_motor_{i}" for i in range(self._num_motor)] + [ f"V_motor_{i}" for i in range(self._num_motor)])
