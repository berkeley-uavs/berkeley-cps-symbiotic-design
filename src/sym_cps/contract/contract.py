...
class ContractManagerBruteForce():
    ...
    def hackthon_get_propeller_property(propeller, table_dict, num_motor):
        ...
        return {"C_p": C_p, "C_t": C_t, "W_prop": W_prop, "diameter": diameter, "shaft_prop": shaft_prop}
    ...
    def hackthon_get_battery_property(battery, num_battery):
        ...
        return {"capacity": capacity, "W_batt": W_batt, "V_battery": V_battery, "I_max": I_max}

    def check_selection(self, num_battery, num_motor, c_library, table_dict, battery, motor, propeller):
        self.hackathon_compose(
    ...
    def hackathon_contract(self, num_battery, num_motor):
        ...
        battery_controller_contract = Contract(
            "batt_controller", battery_controller_name_list, battery_controller_assumption, battery_controller_guarantee
        )
    ...
    def solve(self, c_library: Library):
        ...
        if ret == z3.sat:  # SAT
            ...
            thrust = (
                model[self._vs_dict["system"]["thrust_sum"]].numerator_as_long()
                / model[self._vs_dict["system"]["thrust_sum"]].denominator_as_long()
            )
            weight = (
                model[self._vs_dict["system"]["weight_sum"]].numerator_as_long()
                / model[self._vs_dict["system"]["weight_sum"]].denominator_as_long()
            )
    ...
    def print_metric(self, model):
        ...
        print(
            "    Torque: ",
            model[self._vs_dict["motor"]["torque_motor"]].numerator_as_long()
            / model[self._vs_dict["motor"]["torque_motor"]].denominator_as_long(),
        )
        print(
            "    Torque: ",
            model[self._vs_dict["propeller"]["torque_prop"]].numerator_as_long()
            / model[self._vs_dict["motor"]["torque_motor"]].denominator_as_long(),
        )
        print(
            "    Current Battery: ",
            model[self._vs_dict["battery"]["I_batt"]].numerator_as_long()
            / model[self._vs_dict["battery"]["I_batt"]].denominator_as_long(),
        )
        print(
            "    Current Battery (Motor): ",
            model[self._vs_dict["motor"]["I_motor"]].numerator_as_long()
            / model[self._vs_dict["motor"]["I_motor"]].denominator_as_long(),
        )
        print("    Voltage (Motor): ", model[self._vs_dict["motor"]["V_motor"]])
        print(
            "    Capacity: ",
            model[self._vs_dict["battery"]["capacity"]].numerator_as_long()
            / model[self._vs_dict["battery"]["capacity"]].denominator_as_long(),
        )
        print(
            "    I_max: ",
            model[self._vs_dict["battery"]["I_max"]].numerator_as_long()
            / model[self._vs_dict["battery"]["I_max"]].denominator_as_long(),
        )
        print(
            "    V_battery: ",
            model[self._vs_dict["battery"]["V_battery"]].numerator_as_long()
            / model[self._vs_dict["battery"]["V_battery"]].denominator_as_long(),
        )
        print(
            "    C_t: ",
            model[self._vs_dict["propeller"]["C_t"]].numerator_as_long()
            / model[self._vs_dict["propeller"]["C_t"]].denominator_as_long(),
        )
        print(
            "    C_p: ",
            model[self._vs_dict["propeller"]["C_p"]].numerator_as_long()
            / model[self._vs_dict["propeller"]["C_p"]].denominator_as_long(),
        )
        print(
            "    K_t: ",
            model[self._vs_dict["motor"]["K_t"]].numerator_as_long()
            / model[self._vs_dict["motor"]["K_t"]].denominator_as_long(),
        )
        print(
            "    K_v: ",
            model[self._vs_dict["motor"]["K_v"]].numerator_as_long()
            / model[self._vs_dict["motor"]["K_v"]].denominator_as_long(),
        )
        print(
            "    R_w: ",
            model[self._vs_dict["motor"]["R_w"]].numerator_as_long()
            / model[self._vs_dict["motor"]["R_w"]].denominator_as_long(),
        )
        print(
            "    I_idle: ",
            model[self._vs_dict["motor"]["I_idle"]].numerator_as_long()
            / model[self._vs_dict["motor"]["I_idle"]].denominator_as_long(),
        )
        print(
            "    P_max_motor: ",
            model[self._vs_dict["motor"]["P_max_motor"]].numerator_as_long()
            / model[self._vs_dict["motor"]["P_max_motor"]].denominator_as_long(),
        )
        print(
            "    I_max_motor: ",
            model[self._vs_dict["motor"]["I_max_motor"]].numerator_as_long()
            / model[self._vs_dict["motor"]["I_max_motor"]].denominator_as_long(),
        )
        print(
            "    Diameter: ",
            model[self._vs_dict["propeller"]["diameter"]].numerator_as_long()
            / model[self._vs_dict["propeller"]["diameter"]].denominator_as_long(),
        )
        print("==================================================================")
    ...
    def solve_optimize(self, c_library: Library, max_iter = 0, timeout_millisecond = 100000):
        ...
        while ret == z3.sat:
            ...
            solver.add(
                self._vs_dict["system"]["thrust_sum"] - self._vs_dict["system"]["weight_sum"] >= thrust - weight + 1
            )
            ret = solver.check()
