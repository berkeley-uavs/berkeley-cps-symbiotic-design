...
class MotorPropellerAnalysis(object):
    ...
    def battery_system_analysis_thrust(self, 
                                add_weight = 0,
        ...
        system_motor_connection = [(f"W_motor", f"W_motor"), 
                                    ("I_motor", "I_motor"), 
                                    ("V_motor", "V_motor"),
        ...
        system_batt_connection = [("capacity", "capacity"), 
                                  ("V_battery", "V_battery")]
    ...
    def battery_system_analysis_voltage(self, 
                                add_weight = 0,
                                check_power = False,
                                motors: list[LibraryComponent] = None,
                                propellers: list[LibraryComponent] = None):
    ...
    def set_system_battery_voltage(self, add_weight = 0, check_power = False):
        system_port_name_list = ["rho", "thrust", "weight_sum", "I_motor", "W_motor", "W_prop", "thrust", "V_motor", "capacity", "V_battery",
                                "P_max_motor", "I_max_motor"]
    ...
    def set_system_battery_thrust(self, add_weight = 0):
        ...
        def system_assumption(vs):
            ...
            return [vs["I_motor"] <= vs["capacity"] * 3600 / 400 / 4, vs["weight_sum"] == weight_sum, vs["rho"] == 1.225]
        ...
        def battery_guarantee(vs):
            ...
            for prop_name, (min_val, max_val) in self._battery_dict.items():
                if prop_name == "name": 
                    continue
                clauses.extend(
                                [vs[prop_name] >= min_val, 
                                 vs[prop_name] <= max_val]
                            )
            return clauses
