...
#         General version: each one is different"""
#         self._contracts = {}
...
#             return [vs[f"V_motor_{i}"] <= vs["V_battery"] for i in range(num_battery)]
#         def battery_controller_guarantee(vs, num_battery = num_battery):
#             i_sum = sum([vs[f"I_motor_{i}"] for i in range(num_battery)])
#             return [ i_sum == vs["I_battery"] * 0.95]
#         battery_controller_name_list = ["capacity", "V_battery", "I_battery"]
...
#         """
#         Consider difference in propeller
#         Additional input: motor ratio: this is the ratio to keep aircraft zero moment
#         """
#         #TODO obj_lowerbound has problem, need to give initial value
...
#             motor_ratio = [1] * num_motor
#         if len(motor_ratio) != num_motor:
...
#             self._vs_dict[f"propeller_{i}"] = prop_vs
...
#             self._vs_dict[f"motor_{i}"] = motor_vs
...
#         contr_vs, A, G  = self._contracts["battery_controller"].instantiate(num_battery = num_battery)
...
#             self._prop_select_vs_system.append(prop_select_vs)
#             # set value
#             self._vs_dict[f"prop_{i}_select"] = prop_select_vs
...
#                 use_v = z3.Bool(f"motor_{i}_use_{motor.id}")
...
#             self._vs_dict[f"motor_{i}_select"] = motor_select_vs
#             self._motor_select_vs_system.append(motor_select_vs)
...
#         # connect system
#         # thrust
#         thrust_sum = sum([self._vs_dict[f"propeller_{i}"]["thrust"] for i in range(num_motor)])
#         self._all_clause.append(sys_vs["thrust_sum"] == thrust_sum)
#         weight_sum = battery_vs["W_batt"] + body_weight + sum([self._vs_dict[f"propeller_{i}"]["W_prop"] + self._vs_dict[f"motor_{i}"]["W_motor"] for i in range(num_motor)])
#         self._all_clause.append(sys_vs["weight_sum"] == (weight_sum) * 9.81) # Kg to N
#         # tmp for debug
