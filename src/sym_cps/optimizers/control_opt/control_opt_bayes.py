...
class ControlBayesOptimizer(ControlOptimizer):
    ...
    def optimize_path_grid_speed(self, path = 1, **kwargs):
        ...
        for speed in speeds:
            ...
            def dummy_f(Q_position, Q_velocity, Q_angular_velocity, Q_angles, R):
                ...
                print(f"path = {path}, vs = {vspeed}, ls = {lspeed}, Q_postion = {Q_position}, Q_velocity = {Q_velocity}, Q_angular_velocity = {Q_angular_velocity}, Q_angles = {Q_angles}, R = {R}")
            ...
            pbounds = {name: (self._control_bound[name]["min"], self._control_bound[name]["max"]) for name in self._control_var_names}
    ...
    def optimize_path_all_bayes(self, path = 1, **kwargs):
        ...
        pbounds = {name: (self._control_bound[name]["min"], self._control_bound[name]["max"]) for name in self._control_var_names}
        if path == 4:
            pbounds["speed"] = (self._speed_bound["requested_vertical_speed"]["min"],
                                self._speed_bound["requested_vertical_speed"]["max"])
        else:
            pbounds["speed"] = (self._speed_bound["requested_lateral_speed"]["min"],
