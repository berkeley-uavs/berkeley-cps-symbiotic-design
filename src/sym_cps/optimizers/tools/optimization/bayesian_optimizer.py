...
from sym_cps.optimizers.tools.optimization.util.surrogate import SurrogateInterface, ScikitGPR, LogisticClassifier
...
class BayesianOptimizer(OptimizerBase):
    ...
    def optimize(self, **kwarg):
        ...
        for i in range(self._iteration):
            ...
            # apply the surrogate
            x_min = self._explore_with_surrogate(obj_model=obj_model, 
                                                 con_model=con_model, 
                                                 y_best=y_max_ind_valid, 
                                                 index=i)
