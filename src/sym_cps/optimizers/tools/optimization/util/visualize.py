...
class ContinuousProblemVisualizer(object):
    ...
    def _get_F_1d(self, x_array: npt.ArrayLike):
        ...
        for i, _x in enumerate(x_array):
            obj_vals, con_vals = self._problem.optimize_and_evaluate_script(np.array([_x]))
    ...
    def _plot_objectives_2d(self, idx: int = None):
        if idx is None:
            ...
            c = plt.pcolormesh(
                self._xv, self._yv, self._f_gold[1:, 1:, idx], cmap="coolwarm", shading="flat", label=f"objective {idx}"
            )
            plt.colorbar(c)
