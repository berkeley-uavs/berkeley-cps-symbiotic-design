...
class BayesianOptimizationVisualizer(ContinuousProblemVisualizer):
    ...
    def plot_classification(self, con_model: SurrogateInterface, hist: History, x_next=None):
        ...
        if self._problem.dim == 1:
            ...
            plt.colorbar(c)
            plt.scatter(
                x_explored[:, 0], x_explored[:, 1], color=cm.Greys(np.linspace(0, 1, x_explored.shape[0])), marker="x"
            )
            if x_next is not None:
                plt.scatter(x_next[0], x_next[1], c="cyan", marker="*")
            plt.legend()
            ...
            c = plt.pcolormesh(X, Y, Valid_gold, cmap="coolwarm", shading="nearest")
            plt.colorbar(c)
            plt.scatter(
                x_explored[:, 0], x_explored[:, 1], color=cm.Greys(np.linspace(0, 1, x_explored.shape[0])), marker="x"
            )
    ...
    def plot_prediction(self, obj_model: SurrogateInterface, hist: History, x_next=None):
        ...
        if self._problem.dim == 1:
            ...
            plt.fill_between(X.ravel(), mean.ravel() + 2 * std, mean.ravel() - 2 * std, alpha=0.1)
            plt.plot(X, Y, "y--", lw=1, label="objective")
            ...
            # print(X)
            c = plt.pcolormesh(X, Y, (Mean - F_gold), cmap="coolwarm", shading="nearest")
            plt.colorbar(c)
            plt.scatter(
                x_explored[:, 0], x_explored[:, 1], color=cm.Greys(np.linspace(0, 1, x_explored.shape[0])), marker="x"
            )
