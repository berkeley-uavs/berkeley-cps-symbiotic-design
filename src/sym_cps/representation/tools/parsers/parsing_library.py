        ...
        for entry in info_dict:
            ...
            if component_name not in all_library_components.keys():
                library_component = LibraryComponent(
                    id=component_name, comp_type=all_component_types["unknown"]
                )
                all_component_types["unknown"]._update_field(
                    "belongs_to", {library_component.id: library_component}
                )
            ...
            if "class" in entry.keys():
                ...
                all_component_types["unknown"]._remove_from_field(
                    "belongs_to", library_component.id
                )
                c_type._update_field(
                    "belongs_to", {library_component.id: library_component}
                )
            ...
            if "prop" in entry.keys():
                if "prop_val" in entry.keys():
                    ...
                    library_component._update_field(
                        "properties", {comp_property.name: comp_property}
                    )
                else:
                    pass
        ...
        for entry in info_dict:
            ...
            if "prop" in entry.keys():
                if "prop_val" in entry.keys():
                    ...
                    param_id = parameter_id(
                        entry["prop"], str(library_component.comp_type)
                    )
                    ...
                    library_component.comp_type._update_field(
                        "parameters", {cparam.id: cparam}
                    )
            ...
            if "conn" in entry.keys():
                ...
                if conn_id not in all_connectors.keys():
                    all_connectors[conn_id] = CConnector(
                        entry["conn"], library_component.comp_type
                    )
