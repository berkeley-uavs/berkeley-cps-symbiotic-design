...
from sym_cps.shared.paths import (
    component_library_root_path_default,
    design_library_root_path_default,
)
from sym_cps.representation.tools.parsers.learn_from_designs import (
    parse_connections_and_parameters_from_designs,
)
from sym_cps.representation.tools.parsers.parsing_designs import parse_designs_from_folder


def parse_library_and_seed_designs() -> tuple[
    Library, dict[str, tuple[DConcrete, DTopology]]
]:
    ...
    (
        ...
        design_library_root_path_default, library=c_library
    )

    """Update information to the library"""
    c_library.update_information(
        connectable_connectors, connectable_components_types, design_parameters
    )

    """Load Seed Designs"""
    designs = parse_designs_from_folder(
        design_library_root_path_default, library=c_library
    )
